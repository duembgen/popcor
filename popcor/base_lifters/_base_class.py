...
from popcor.utils.common import create_symmetric, get_labels, get_vec
from popcor.utils.constraint import Constraint, remove_dependent_constraints
...
class BaseClass(object):
    ...
    EPS_ERROR: float = 1e-8

    PARAM_LEVELS: list[str] = ["no", "p", "ppT"]

    theta_: np.ndarray | None = None
    parameters_: dict | None = None
    ...
    @staticmethod
    def get_variable_indices(var_subset: Iterable, variable: str = "z") -> np.ndarray:
        return np.unique(
            [int(v.split("_")[-1]) for v in var_subset if v.startswith(f"{variable}_")]
        )

    def __init__(self, d: int, param_level: str, n_parameters: int) -> None:
    ...
    def get_var_dict(
        self, var_subset: Iterable | None = None, unroll_keys: bool = False
    ) -> dict:
    ...
    def get_param_dict(self, param_subset: Iterable | None = None) -> dict:
    ...
    @theta.setter
    def theta(self, t: np.ndarray) -> None:
        assert (
            self.theta_ is None
    ...
    def extract_A_known(
        self, A_known: list, var_subset: dict | list, output_type: str = "csc"
    ) -> list | np.ndarray:
        """
        ...
        for A in A_known:
            ...
            if np.all([v in var_subset for v in A_poly.get_variables()]):
                ...
                if output_type == "dense":
                    a = get_vec(Ai, correct=True)
                    assert isinstance(a, np.ndarray)
                    ai = self.augment_using_zero_padding(a, var_subset)
    ...
    def get_param_idx_dict(self, var_subset: Iterable) -> dict:
        """
    ...
    def get_mat(
        ...
        correct: bool = True,
    ) -> np.ndarray | sp.csr_matrix | sp.csc_matrix:
    ...
    def var_list_row(
        self,
        var_subset: list | dict | None = None,
        param_subset: list | dict | None = None,
        force_parameters_off: bool = False,
    ) -> list[str]:
        if var_subset is None:
            var_subset = list(self.var_dict.keys())
        elif isinstance(var_subset, dict):
            var_subset = list(var_subset.keys())
        assert var_subset is not None
        ...
        if force_parameters_off:
            ...
            param_dict = unroll(self.get_param_dict(param_subset))
    ...
    def var_dict_row(
        self, var_subset: list | dict | None = None, force_parameters_off: bool = False
    ) -> dict:
        return {
            label: 1
    ...
    def get_basis_from_poly_rows(
        self, basis_poly_list: list, var_subset: list | dict | None = None
    ) -> np.ndarray:
    ...
    def convert_polyrow_to_Apoly(
        self, poly_row: PolyMatrix, correct: bool = True
    ) -> PolyMatrix:
    ...
    def convert_polyrow_to_Asparse(
        self, poly_row: PolyMatrix, var_subset: Iterable | None = None
        ...
        assert isinstance(mat_sparse, (sp.csr_matrix, sp.csc_matrix))
        return mat_sparse

    def convert_polyrow_to_a(
        self,
        poly_row: PolyMatrix,
    ...
    def get_dim_x(self, var_subset: Iterable | None = None) -> int:
        var_dict = self.get_var_dict(var_subset)
        return sum([val for val in var_dict.values()])

    def get_dim_p(self, param_subset: Iterable | None = None) -> int:
    ...
    def get_dim_P(self, param_subset: list | dict | None = None) -> int:
        return len(self.get_p(param_subset=param_subset))

    def get_reduced_a(
        self,
        bi: np.ndarray | PolyMatrix | sp.csc_matrix | sp.csr_matrix,
        param_here: np.ndarray | None = None,
        var_subset: list | dict | None = None,
        param_subset: list | dict | None = None,
        sparse: bool = False,
        ...
        if isinstance(bi, np.ndarray):
            ...
            var_dict_row = self.var_dict_row(var_subset)
            bi = bi.get_matrix(([self.HOM], var_dict_row))
            len_b = bi.shape[1]
        else:
            len_b = bi.shape[1]
    ...
    def augment_using_zero_padding(
        self, ai: np.ndarray, param_subset: list | dict | None = None
    ) -> np.ndarray:
        n_parameters = self.get_dim_P(param_subset)
        return np.hstack([ai, np.zeros((n_parameters - 1) * len(ai))])

    def augment_using_parameters(
        self, x: np.ndarray, param_subset: dict | None = None
    ) -> np.ndarray:
    ...
    def get_constraint_rank(
        self,
        A_list_poly: list,
        b_known: list | None = None,
    ...
    def apply_template(
        ...
        new_poly_rows: list[PolyMatrix] = []
        unique_idx: set[int] = set()
    ...
    def get_vec_around_gt(self, delta: float = 0) -> np.ndarray:
    ...
    def get_A0(self, var_subset: Iterable | None = None) -> tuple[list, list]:
        if var_subset is not None:
    ...
    def sample_parameters_landmarks(self, landmarks: np.ndarray) -> dict:
        ...
        for i in range(n_landmarks):
            if self.param_level == "p":
                ...
                quadratic_terms = get_vec(
                    np.outer(landmarks[i], landmarks[i]),
                    correct=False,
                    sparse=False,
                )
                parameters[f"p_{i}"] = np.hstack(  # type: ignore
                    [landmarks[i], np.asarray(quadratic_terms)]
                )
        return parameters

    def get_error(
        self, theta_hat: np.ndarray, error_type: str = "mse", *args, **kwargs
    ) -> float:
    ...
    def get_p(
        self,
        parameters: dict | None = None,
        param_subset: dict | list | None = None,
    ) -> np.ndarray:
